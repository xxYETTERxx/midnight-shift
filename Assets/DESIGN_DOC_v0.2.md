...
This is a forward-looking design reference, not a build log. Feed it to a new chat instance to get back up to speed. It captures the high-level concept, the systems we've designed, the technical approach, and the open questions. It does not capture art assets created, code written, or specific tile layouts — those live in the project itself.
...
### Time structure
- **Day length:** Stardew-style. Wake at 6am, pass out at 2am.
- **Real-time:** ~1 in-game minute = ~0.7 real seconds (tunable). Full day ≈ 14 real minutes.
- **Pause** in menus, dialogue, minigames.

### Minute-to-minute (typical day)

The pager loop and hangouts now make this much richer than v0.1. Example day:

| Time | Activity |
|---|---|
| 12:00 | Wake, water plants, eat |
| 1:00 | Bodega for supplies, chat with shopkeeper |
| 2:30 | **Pager goes off** — regular wants an eighth at noon |
| 3:00 | Travel to payphone, return call, set the meet |
| 4:00 | Drop yesterday's harvest at the fence |
| 5:00 | Meet buyer, $80 |
| 6:00 | Travel to affluent neighborhood, case a house for an hour |
| 7:00 | Home, harvest mature plants, start new |
| 8:00 | Eat |
| 10:00 | Romance interest is at the diner — go talk |
| 11:00 | **Pager** — unknown number, sketchy vibe |
| 12:30 | Take the risk, gas station meet, $120 but felt off |
| 1:45 | Hit the house you cased? Or quit while ahead? |
| 4:00 | Home, sleep |
...
**Tuning principle 2:** The schedule should *almost* fit, not comfortably fit. If a productive day is ~20 in-game hours, the player's want-to-do list should average 24-28 hours. The 4-8 hour overflow is what makes them want to play tomorrow.

---

## 7. The Pager Dealing Loop

This is now the **schedule-disrupting engine** of the game. It's not just income — it's the system that creates moment-to-moment dramatic choices. Promoted from "post-MVP polish" to a core MVP system because it's central to making days feel alive.
...
The player's apartment has space for a few growing pots. Full chain:
...
- Light source (basic lamp = baseline, grow lamp = +quality, requires upgrade)
- Strain (post-MVP — start with one)
...
### Mechanic style
Stardew-direct. Held item + interactable container = action. No menus. Soil bag on empty pot, seed on soiled pot, watering can on seeded pot, click mature plant to harvest. Muscle memory; *interest* comes from balancing water cycles against everything else competing for your day.
...
### Minigames
- Lockpicking (skill-gated, mechanic TBD — explicit open question, see §17)
...
Stardew-style cutscenes triggered by location + time + precondition. They can `pushDialogue()` follow-up lines onto an NPC's stack. Don't author events yet, but the dialogue system must support push-stack from day one.
...
### Hangouts (NEW — promoted to MVP-relevant)
...
**Handcrafted:**
1. Player's Apartment — bed, kitchen, growing area, storage
...
**Engine:** Godot 4.3+ (use `TileMapLayer`, not legacy `TileMap`).
...
│   └── minigames/
├── scripts/
│   ├── systems/           # time, economy, heat, relationships, save, pager
...
│   ├── items.json
│   ├── npcs.json
│   ├── buyers.json        # pager-dealing recurring buyers
│   ├── schedules/
│   ├── loot_tables/
│   └── room_templates/
└── art/
...
- **Data-driven everything** — items, dialogue, schedules, loot tables, NPC and buyer definitions in editable text/Resource files
- **Hot reload during dev** — `F5` reloads dialogue/items/schedules without restarting
- **Singletons (Autoloads):** `TimeSystem`, `EconomySystem`, `RelationshipSystem`, `HeatSystem`, `SaveSystem`, `DialogueSystem`, `PagerSystem`
- **Signals for cross-system events** — crime committed → emit signal → heat/relationships/dialogue all react
- **Save system** = singletons + per-scene state (NPC positions, plant states, container contents) → single file. Build from day one.

### Tilemap conventions
- 32×32 tiles, tileset PNG laid out on a 512×512 canvas (room to grow without repositioning existing tiles)
- Walls split across two rows: bottom row = walkable "wall base," top row = blocked. Standing-against-wall illusion via Y-sort
...
- Y-sort enabled on room roots; sprite pivots at *base* of object (character feet, tree trunk)
...
1. **Stamina granularity** — points (Stardew energy) or continuous bar?
...
### Stage 2: Time and State
- Day/night clock
- Sleep/wake cycle
- Save/load (build now, not later — retrofitting saves is painful)
- Stamina meter

### Stage 3: First Income Loop (Weed)
- Inventory system
- Held-item interactions (item + container = action)
...
If all twelve are achievable in a vertical slice, MVP is done and "is this loop fun?" can be answered with real playtest data.
