...
**Changes from v0.3:** New §23 "Neighborhood Progression System" covers the two-track (community vs. criminal) overarching goal structure, neighborhood slots, and the three timescales of player feedback. §5 updated with pawn shop / catalogue equipment pipeline and pager-rep gating. §6 updated with neighborhood investment as a fifth day-filling category. §12 updated to map cast onto the two-track tension. §13 updated with neighborhood slot locations. §16, §18, §19, §20 updated to reflect the new system.
...
Two channels for acquiring equipment and upgrades, each with different inventory and friction:

- **Pawn shop** (in-person, cash): rotating daily stock. Basics always available (soil, seeds, baggies, lockpicks). Mid-tier items (grow lamps, planter beds) appear once the player has enough pager rep — the Fence stocks what makes sense for your level of operation. Stock rotates, so if you see something you want, buy it now or wait for it to cycle back.
- **Mail-order catalogue** (order + wait): higher-tier equipment not available at the pawn shop. Player sends a check (physical cash committed), waits 2-3 days for delivery. This creates a planning horizon — you commit money now for capability later — and a reason to check your mailbox. Catalogue access and selection expand with pager rep.

**Pager rep gates progression.** The player's dealing reputation (buyer network size, successful deals, product quality) is the spine that unlocks access to better equipment, new strains, and eventually neighborhood-level opportunities (see §23). More rep → more contacts → better stock at the pawn shop → deeper catalogue. This makes the pager loop the central progression mechanic, not just an income source.

### Dirty vs. clean cash
Deferred post-MVP, but architect the wallet to support multiple currency pools so it's not a refactor later. The mail-order system implies a distinction between physical cash and some form of sendable payment — keep this lightweight for MVP (just deduct from wallet) but flag for post-MVP expansion into money laundering and bank accounts.
...
Every activity falls into one of five time-cost categories:
...
5. **Neighborhood investment** — multi-day/week projects that shape the world (see §23). These compete directly with income and social time, creating the game's central tension: do you spend tonight making money, building a relationship, or building the neighborhood?
...
**Cast alignment with the two-track system (§23):** The cast naturally splits across the community/criminal tension. The Fence and the Mentor pull toward the criminal path — spending time with them deepens criminal capability. The Romance and the Straight-World Anchor pull toward the community path — their questlines and hangouts invest in the neighborhood's legitimate future. The Rival creates pressure from whichever direction the player *isn't* going (criminal rival if you go community, community opposition if you go criminal). The Wildcard fills whatever gap emerges. This means time spent on relationships is implicitly time spent drifting toward one pole or the other — social and neighborhood-investment categories overlap in important ways.
...
2. The Neighborhood (street + adjacent buildings) — includes **neighborhood slots** (see §23): the empty lot, the bodega facade, the street lighting, the alley, etc. These are the physical locations that visually transform based on the player's community/criminal choices.
3. The Bar / Diner — late-night socializing
4. The Bodega / Corner Store — supplies, food, Straight-World Anchor's workplace. Also a neighborhood slot: its condition and stock reflect whether the player helped fight off the mob or took over the extortion.
5. The Fence's Spot — pawn shop back room. Also the equipment shop (see §5).
...
- Money laundering / bank accounts (build wallet to support multiple pools; mail-order system hints at this)
...
- Story arcs and named events (system supports them; no content authored — but §23 neighborhood progression provides implicit arc structure)
...
- Extortion as income source (criminal-path neighborhood slot, see §23 — requires NPC schedules and economy hooks)
- Police informant questline (late-game community path, requires 5+ community slots flipped — see §23)
- Neighbourhood expansion beyond the home neighbourhood (criminal path endgame)
...
13. **Neighborhood slot count for MVP** — 8-10 is the design target (see §23), but even 4-5 well-authored slots might be enough to prove the system. How many is "enough to feel like a real choice"?
14. **Neighborhood drift visibility** — how aggressively should the street change? Subtle (lighting, NPC presence, ambient dialogue shifts) vs. dramatic (new structures, blocked paths, tileset swaps)? Probably start subtle, escalate.
15. **Slot progression requirements** — should every slot require multi-system engagement (cash + relationship + item), or should some be simple cash-only to establish the pattern before complexity ramps?
16. **Criminal path income balance** — extortion and protection money need to feel meaningfully different from pager dealing income, or the criminal path is just "more of the same with goons." What's the mechanical distinction?
...
- 4-5 neighborhood slots with default/community/criminal states
...
11. Encounter at least one neighborhood slot event, make a community-or-criminal choice, and see the physical result on the street
12. Purchase equipment through both the pawn shop and mail-order catalogue
13. Survive (or fail to survive) a full in-game month
14. Save and reload without state loss

If all fourteen are achievable in a vertical slice, MVP is done and "is this loop fun?" can be answered with real playtest data.
...
This is the overarching goal structure — the thing that gives the daily loop a direction and an ending. Without it, the game is a treadmill. With it, the player is building something visible, and every day's choices accumulate into a neighborhood that reflects who they decided to be.

### The core tension

The player's neighborhood is caught between two forces: **organized crime** (mob extortion, protection rackets, neglect) and **institutional neglect** (cops who don't care, infrastructure that's crumbling, no investment). The player can push back against either — or exploit either.

**Community path:** fight the mob's grip on the neighborhood, invest in local businesses and infrastructure, build trust with straight-world residents. The neighborhood gets safer, cleaner, more livable. The player becomes a respected local figure who happens to sell weed on the side.

**Criminal path:** replace the mob's operation with your own, build an empire, recruit muscle, take over businesses as fronts. The neighborhood becomes *yours* — more dangerous for outsiders, more profitable for you. The player becomes the new power.

**These are not binary routes.** The player drifts between them based on cumulative daily choices. Help the bodega owner fight off extortion one week, then shake down another shop the next. The neighborhood's state reflects the *net* of all choices, not a single branching decision. But the late-game content does gate behind thresholds — go far enough in one direction and certain opportunities open that wouldn't otherwise.

### Neighborhood slots

The neighborhood has **8-10 physical slots** — specific locations or features that can exist in one of three states: **degraded** (default), **community**, or **criminal**. Each slot is a real place in the game world that the player walks past regularly.

**Example slots** (not final — pick during authoring based on what the street layout supports):

| # | Slot | Degraded (default) | Community path | Criminal path |
|---|---|---|---|---|
| 1 | The empty lot | Trash, broken glass, weeds | Basketball court; kids and NPCs hang out | Chop shop; fences stolen vehicles at good rates |
| 2 | The bodega | Mob extortion, thin shelves, scared owner | Thriving shop, better stock, specialty seeds | Player-run extortion, weekly cash drop, owner hostile |
| 3 | Street lighting | Half the streetlights broken | Repaired lights; safer at night, lower area heat | Lights *stay* broken; darkness benefits your operations |
| 4 | The abandoned storefront | Boarded up, unusable | Community space (laundromat? clinic?) — new NPC | Front business for laundering (post-MVP mechanic hook) |
| 5 | The apartment building facade | Graffiti, broken buzzer, leak in your ceiling | Repairs, better security, apartment upgrade unlocked | Reinforced door, stash room, grow room expansion |
| 6 | The alley behind the bar | Sketchy, mugging risk | Cleaned up; safe shortcut, small garden | Lookout post; your guys watch your back |
| 7 | The payphone corner | Just a payphone | Bench, better lighting, NPC foot traffic | Dead drop location; pager deals are faster here |
| 8 | The park / green space | Overgrown, unused | Maintained; hangout spot, relationship boost zone | Meeting ground; criminal contacts find you here |

The exact slots are authored during Stage 10 based on what the tileset and street layout support. The table above is a starting framework for design — some will merge, some will split, some new ones will emerge.

### How slots change

Slots don't flip via a menu. They change through **scripted events** that surface naturally through gameplay.

**Surfacing pattern:**
1. **Seed:** An NPC mentions the slot in dialogue. The Straight-World Anchor says the bodega owner is being squeezed. The Mentor mentions someone's been dumping cars in the lot. The player walks past the slot and notices it exists, but can't act yet.
2. **Trigger:** A specific condition is met — relationship threshold with the relevant NPC, pager rep level, day count, or completion of a prior slot. The event fires: a dialogue scene, a confrontation, a request for help.
3. **Choice:** The player is presented with the fork. Not always explicitly — sometimes the choice is which NPC's request you prioritize, or whether you show up to help or ignore the page.
4. **Execution:** The chosen path has a **multi-day or multi-week goal requirement** before the slot actually flips. This is where the day-filling principle bites: completing a slot means spending evenings gathering materials, doing favors, running specific errands, or executing specific crimes — all competing with your other obligations.
5. **Payoff:** The slot flips. The street visually changes. New mechanical benefits unlock. NPCs react in dialogue.

### Goal requirements (the day-filling hook)

Slot goals should pull from **multiple systems** to force cross-system engagement. Not every slot needs maximum complexity — early slots can be simpler to teach the pattern — but the mid-to-late slots should require the player to juggle.

**Example: The empty lot → basketball court (community)**
- $300 cash (economic pressure)
- 3 visits to the hardware store / catalogue orders for materials (time + planning)
- Straight-World Anchor relationship ≥ 40 affinity (social investment)
- Spend 2 evenings on-site during "construction" (direct time cost, competes with income)

**Example: The empty lot → chop shop (criminal)**
- $500 cash (higher economic cost — criminal infrastructure is expensive)
- Fence relationship ≥ 50 trust (criminal social investment)
- Successfully complete 2 B&Es (proves capability)
- Recruit a contact through pager network (rep threshold)

The key: both paths cost time and money, but they cost *different kinds* of time and money, pulling the player toward different parts of the game.

### Three timescales of feedback

The neighborhood should feel alive and reactive at every scale, not just when a slot flips.

**Daily (ambient):** Small, cheap-to-author reactions that accumulate. An NPC waves who didn't before. A lookout nods as you pass. The bodega owner leaves something behind the counter. Ambient dialogue shifts — neighbors gossip about what you've been up to. These are dialogue-key swaps and NPC presence/absence toggles, not new art.

**Weekly (environmental):** Visible but incremental changes. A boarded window gets glass. A new piece of street art appears. An NPC starts hanging out at a new location. Your apartment gets a small upgrade. These are tile swaps and NPC schedule changes — noticeable when you're paying attention, accumulating into atmosphere.

**Monthly (landmark):** Slot flips. The lot transforms. A new location opens. The street's character shifts. These are the big "I did that" moments that justify the month of work. They should feel earned and be visually unambiguous — the player should be able to screenshot the before and after.

### Drift tracking

Internally, the neighborhood tracks two scores:

- **Community score** (0-100): sum of community-path progress across all slots
- **Criminal score** (0-100): sum of criminal-path progress across all slots

These aren't directly visible to the player as numbers — they're reflected in the world state, NPC dialogue, and what opportunities surface. But internally they gate content:

- **Community ≥ 30:** straight-world NPCs start actively helping you (tips about police schedules, lookout for your grow op)
- **Community ≥ 60:** informant questline becomes available (working with a sympathetic cop to clean up the mob — late-game, post-MVP)
- **Criminal ≥ 30:** muscle shows up (goons near your apartment who intervene in conflicts)
- **Criminal ≥ 60:** territory expansion (ability to muscle into the adjacent neighborhood — late-game, post-MVP)

The two scores are **not** mutually exclusive in the early game — you can do some of each. But the late-game thresholds effectively require commitment, because there aren't enough slots to max both. A player who splits evenly ends up around 40/40, which gets some benefits from each side but can't access the deepest content on either. This is intentional — the "middle path" is viable but doesn't get the biggest payoffs. The game rewards commitment without punishing moderation.

### Integration with existing systems

**Pager rep** is the gatekeeper. Slots don't surface until the player has enough dealing reputation to be a meaningful player in the neighborhood. This prevents the progression system from overwhelming early game, and ensures the player has the core loop established before the overarching structure kicks in. First slot event fires around end of week 1 or start of week 2.

**Relationship system:** Many slot goals require relationship thresholds with specific NPCs. This means the player can't ignore the social layer and still progress the neighborhood — hangouts and gifts aren't just for flavor, they're prerequisites.

**Heat system:** Criminal-path slots tend to raise area heat during execution. Community-path slots tend to lower it over time once complete. This creates a natural feedback loop: going criminal makes the neighborhood hotter, which makes everything harder, which incentivizes going deeper criminal to get the muscle and infrastructure to handle it. Going community makes the neighborhood calmer, which makes legitimate activities easier, but also makes your grow op more conspicuous by contrast.

**Economy:** Slot goals cost money. The player has to balance slot investment against rent, supplies, and equipment upgrades. A slot goal requiring $500 when rent is due Sunday is a real tension.

### MVP scope for neighborhood progression

For MVP, the goal is to **prove the system works and feels good**, not to author all 8-10 slots to completion. Specifically:

- Implement the drift tracking system (community/criminal scores, slot state machine)
- Author **3-4 slots** with full community + criminal paths, scripted events, goal requirements, and visual payoffs
- At least one slot should be completable within the first in-game month
- The remaining 4-6 slots can exist as degraded-only placeholders, with events and paths authored post-MVP
- Daily and weekly ambient feedback should cover the whole neighborhood, not just completed slots — the vibe shifts even before specific slots flip
